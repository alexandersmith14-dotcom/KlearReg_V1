...
.hero-word{position:relative;font-family:inherit;font-weight:800;
  font-size:clamp(46px,7.5vw,88px);color:#fff;margin:0;line-height:1;
  letter-spacing:-.02em;display:flex;align-items:flex-end}
.hero-word-text{position:relative;z-index:1;display:flex;align-items:flex-end}
/* The Check Spike mark, stretched (preserveAspectRatio="none" on the
   element) to span the full wordmark instead of sitting as a small
   trailing icon — the tick starts in front of the K, the long stroke
   sweeps across to "ance", stamped in front of the letters (z-index above
   .hero-word-text) once they've landed. Same path/viewBox as every other
   Check Spike usage on the page; only the container's aspect ratio
   changes; stroke-dasharray/offset operate in the path's own
   pre-stretch viewBox units, not rendered pixels, so the draw-length
   values below don't need to change with the stretch. */
.hero-checkmark{position:absolute;inset:0;width:100%;height:100%;z-index:2;
  pointer-events:none;overflow:visible}
.hero-checkmark path{stroke-dasharray:60;stroke-dashoffset:60;
  animation:krCheckDraw .8s cubic-bezier(.65,0,.35,1) 1.5s forwards}
@keyframes krCheckDraw{to{stroke-dashoffset:0}}
.hero-checkmark .hero-ping-sm{animation:heroPing 2.6s cubic-bezier(.25,.6,.4,1) 2.3s infinite}
.hero-checkmark circle:last-child{opacity:0;animation:krDotIn .15s ease-out 2.3s forwards}
@keyframes krDotIn{to{opacity:1}}
/* K/R merge: the wordmark's two brand-color letters start pulled apart
   ...
   rather than just a slide-and-stop. The checkmark sweep (above) starts
   right after, at 1.5s, so the sequence reads as one beat: letters merge,
   then get checked off. Implicit 0% keyframe = the element's own
   pre-animation transform (set by .kr-k/.kr-r below), so krMerge only
   needs a `to` frame. */
...
@media (prefers-reduced-motion:reduce){
  ...
  .hero-checkmark path{animation:none;stroke-dashoffset:0}
  .hero-checkmark .hero-ping-sm{animation:none;opacity:0}
  .hero-checkmark circle:last-child{animation:none;opacity:1}
}
...
@media (max-width:820px){
  /* padding/gap tightened well below the desktop values (44px/44px) rather
     ...
     (ported from Mihari) -- an earlier pass cut bottom to 10px while leaving
     top at 28px, which traded the dead-space problem for a lopsided
     top-heavy look on a real device; this keeps both ends trim without
     either one reading as bigger than the other. herowrap's own
...
<div class="herowrap">
  ...
  <div class="hero-inner">
    <div class="hero-titleblock">
      <p class="hero-word"><svg class="hero-checkmark" viewBox="0 0 40 34" preserveAspectRatio="none" aria-hidden="true">
        ...
        </svg><span class="hero-word-text"><span class="kr kr-k">K</span>lea<span class="kr kr-r">R</span>ance</span></p>
